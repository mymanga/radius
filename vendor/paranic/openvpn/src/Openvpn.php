...
class Openvpn {
	...
	public $protocol = 'udp';
	public $easy_rsa_folder = '/var/www/html/public/easy-rsa';
	...
	public function delete_client($client_name = NULL)
	{
		if (!$this->isEasyRSASetup()) die('Please invoke setup() first.' . PHP_EOL);

		// Check client_name
		if (is_null($client_name) OR empty($client_name)) die('Please input client name that you want to delete.' . PHP_EOL);

		// Check if client_name exists in our keys database
		if (!in_array($client_name . '.conf', $this->existing_keys())) die('Client name does not exist in database.' . PHP_EOL);

		shell_exec('cd ' . $this->easy_rsa_folder . ' && ./easyrsa --batch revoke ' . $client_name);
		shell_exec('cd ' . $this->easy_rsa_folder . ' && ./easyrsa --batch gen-crl');

		// Remove the client configuration file
		unlink($this->easy_rsa_folder . '/configs/' . $client_name . '.conf');

		// Remove the client CCD file
		unlink($this->easy_rsa_folder . '/ccd/' . $client_name);
