...
<html lang="en">
   <head>
      ...
      <link href="{{ URL::asset('assets/hotspot/clean/clean.css') }}" rel="stylesheet">
   </head>
   <body>
      <nav class="navbar">
         <div class="container">
      ...
      <div class="container">
         ...
         <div class="panel">
            ...
            <form name="login" id="login-form" action="{{$common_data['link_login_only']}}" method="post" onsubmit="return doLogin()" rel="noreferrer">
               ...
               {{-- 
               <p class="text-center">Please log in to use the internet hotspot service</p>
               --}}
               ...
               <div>
                  <button class="btn btn-buy mt-3" type="submit">Connect</button>
               </div>
               ...
               <div class="col-12 mb-3">
                  <div class="card">
                     <div class="card-body text-center">
                        ...
                        @endphp
                        <p class="card-text">Price: <del class="text-muted">Ksh {{$plan->price}}</del> <span class="text-danger">Ksh {{$discounted_price}}</span></p>
                        <p class="text-success">You save {{$plan->discount_rate}}%!</p>
                        @else
                        <p class="card-text">Price: <span class="text-primary">Ksh {{$plan->price}}</span></p>
                        @endif
                        @if($plan->speed_limit)
                        ...
                        @endif
                        <button type="button" class="btn btn-buy" data-bs-toggle="modal" data-bs-target="#purchaseVoucherModal" data-title="{{$plan->title}}" data-amount="{{ $discounted_price ?? $plan->price }}" data-plan-id="{{$plan->id}}">
                        PURCHASE
                        </button>
                     </div>
                  </div>
            ...
            <div class="modal-content">
               ...
               <form action="{{route('voucher.buy')}}" method="POST">
                  ...
                  <div class="modal-footer">
                     <button type="submit" class="btn btn-buy">Purchase Voucher</button>
                  </div>
               </form>
            </div>
         </div>
      ...
      <div class="modal fade" id="successModal" tabindex="-1" aria-labelledby="successModalLabel" aria-hidden="true">
         <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
               ...
               <div class="modal-footer">
                  <button type="button" class="btn btn-buy" data-dismiss="modal">OK</button>
               </div>
            </div>
         </div>
      </div>
